// Package docs Code generated by swaggo/swag. DO NOT EDIT
package docs

import "github.com/swaggo/swag"

const docTemplate = `{
    "schemes": {{ marshal .Schemes }},
    "swagger": "2.0",
    "info": {
        "description": "{{escape .Description}}",
        "title": "{{.Title}}",
        "contact": {},
        "version": "{{.Version}}"
    },
    "host": "{{.Host}}",
    "basePath": "{{.BasePath}}",
    "paths": {
        "/admin-api/app/health": {
            "get": {
                "description": "Health endpoint for deployment probes and service-governance checks.",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Health"
                ],
                "summary": "Module health",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_lohasle_nimbus-cloud-framework-go_internal_httpx.Response"
                        }
                    }
                }
            }
        },
        "/admin-api/application/health": {
            "get": {
                "description": "Health endpoint for deployment probes and service-governance checks.",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Health"
                ],
                "summary": "Module health",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_lohasle_nimbus-cloud-framework-go_internal_httpx.Response"
                        }
                    }
                }
            }
        },
        "/admin-api/business/health": {
            "get": {
                "description": "Health endpoint for deployment probes and service-governance checks.",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Health"
                ],
                "summary": "Module health",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_lohasle_nimbus-cloud-framework-go_internal_httpx.Response"
                        }
                    }
                }
            }
        },
        "/admin-api/im/health": {
            "get": {
                "description": "Health endpoint for deployment probes and service-governance checks.",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Health"
                ],
                "summary": "Module health",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_lohasle_nimbus-cloud-framework-go_internal_httpx.Response"
                        }
                    }
                }
            }
        },
        "/admin-api/infra/health": {
            "get": {
                "description": "Health endpoint for deployment probes and service-governance checks.",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Health"
                ],
                "summary": "Module health",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_lohasle_nimbus-cloud-framework-go_internal_httpx.Response"
                        }
                    }
                }
            }
        },
        "/admin-api/member/health": {
            "get": {
                "description": "Health endpoint for deployment probes and service-governance checks.",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Health"
                ],
                "summary": "Module health",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_lohasle_nimbus-cloud-framework-go_internal_httpx.Response"
                        }
                    }
                }
            }
        },
        "/admin-api/pay/health": {
            "get": {
                "description": "Health endpoint for deployment probes and service-governance checks.",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Health"
                ],
                "summary": "Module health",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_lohasle_nimbus-cloud-framework-go_internal_httpx.Response"
                        }
                    }
                }
            }
        },
        "/app/health": {
            "get": {
                "description": "Health endpoint used by deployment probes and scaffold modules.",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Health"
                ],
                "summary": "Service health",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_lohasle_nimbus-cloud-framework-go_internal_httpx.Response"
                        }
                    }
                }
            }
        },
        "/application/health": {
            "get": {
                "description": "Health endpoint used by deployment probes and scaffold modules.",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Health"
                ],
                "summary": "Service health",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_lohasle_nimbus-cloud-framework-go_internal_httpx.Response"
                        }
                    }
                }
            }
        },
        "/health": {
            "get": {
                "description": "Health endpoint used by deployment probes and scaffold modules.",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Health"
                ],
                "summary": "Service health",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_lohasle_nimbus-cloud-framework-go_internal_httpx.Response"
                        }
                    }
                }
            }
        },
        "/im/health": {
            "get": {
                "description": "Health endpoint used by deployment probes and scaffold modules.",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Health"
                ],
                "summary": "Service health",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_lohasle_nimbus-cloud-framework-go_internal_httpx.Response"
                        }
                    }
                }
            }
        },
        "/infra/health": {
            "get": {
                "description": "Health endpoint used by deployment probes and scaffold modules.",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Health"
                ],
                "summary": "Service health",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_lohasle_nimbus-cloud-framework-go_internal_httpx.Response"
                        }
                    }
                }
            }
        },
        "/member/health": {
            "get": {
                "description": "Health endpoint used by deployment probes and scaffold modules.",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Health"
                ],
                "summary": "Service health",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_lohasle_nimbus-cloud-framework-go_internal_httpx.Response"
                        }
                    }
                }
            }
        },
        "/pay/health": {
            "get": {
                "description": "Health endpoint used by deployment probes and scaffold modules.",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Health"
                ],
                "summary": "Service health",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_lohasle_nimbus-cloud-framework-go_internal_httpx.Response"
                        }
                    }
                }
            }
        },
        "/system/auth/get-permission-info": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Returns current admin profile, roles, permissions and menu tree.",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "System Auth"
                ],
                "summary": "Current user and menus",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_lohasle_nimbus-cloud-framework-go_internal_httpx.Response"
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "$ref": "#/definitions/github_com_lohasle_nimbus-cloud-framework-go_internal_httpx.Response"
                        }
                    }
                }
            }
        },
        "/system/auth/login": {
            "post": {
                "description": "Authenticates an operations-console user in a tenant.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "System Auth"
                ],
                "summary": "Admin login",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Tenant ID",
                        "name": "tenant-id",
                        "in": "header",
                        "required": true
                    },
                    {
                        "description": "Credentials",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/internal_system.LoginRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_lohasle_nimbus-cloud-framework-go_internal_httpx.Response"
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "$ref": "#/definitions/github_com_lohasle_nimbus-cloud-framework-go_internal_httpx.Response"
                        }
                    }
                }
            }
        },
        "/system/auth/logout": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Ends the client session. JWT tokens expire naturally in this scaffold.",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "System Auth"
                ],
                "summary": "Admin logout",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_lohasle_nimbus-cloud-framework-go_internal_httpx.Response"
                        }
                    }
                }
            }
        },
        "/system/health": {
            "get": {
                "description": "Health endpoint used by deployment probes and scaffold modules.",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Health"
                ],
                "summary": "Service health",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_lohasle_nimbus-cloud-framework-go_internal_httpx.Response"
                        }
                    }
                }
            }
        },
        "/system/tenant/get-id-by-name": {
            "get": {
                "description": "Returns an enabled tenant ID by tenant name.",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "System Auth"
                ],
                "summary": "Resolve tenant ID",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Tenant name",
                        "name": "name",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_lohasle_nimbus-cloud-framework-go_internal_httpx.Response"
                        }
                    }
                }
            }
        }
    },
    "definitions": {
        "github_com_lohasle_nimbus-cloud-framework-go_internal_httpx.Response": {
            "type": "object",
            "properties": {
                "code": {
                    "type": "integer",
                    "example": 0
                },
                "data": {},
                "msg": {
                    "type": "string",
                    "example": ""
                }
            }
        },
        "internal_system.LoginRequest": {
            "type": "object",
            "required": [
                "password",
                "username"
            ],
            "properties": {
                "password": {
                    "type": "string",
                    "example": "admin123"
                },
                "username": {
                    "type": "string",
                    "example": "admin"
                }
            }
        }
    },
    "securityDefinitions": {
        "BearerAuth": {
            "type": "apiKey",
            "name": "Authorization",
            "in": "header"
        }
    }
}`

// SwaggerInfo holds exported Swagger Info so clients can modify it
var SwaggerInfo = &swag.Spec{
	Version:          "1.0",
	Host:             "",
	BasePath:         "/admin-api",
	Schemes:          []string{},
	Title:            "Nimbus Cloud Framework Go System API",
	Description:      "Tenant, operations-user and authentication service.",
	InfoInstanceName: "swagger",
	SwaggerTemplate:  docTemplate,
	LeftDelim:        "{{",
	RightDelim:       "}}",
}

func init() {
	swag.Register(SwaggerInfo.InstanceName(), SwaggerInfo)
}
