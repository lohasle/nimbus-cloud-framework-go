...
func (h *Handler) AccessLogPage(c *gin.Context) {
	...
	for _, row := range rows {
		...
		views = append(views, gin.H{
			...
			"applicationName": applicationName, "requestMethod": row.Method, "requestUrl": row.Path,
...
func (h *Handler) accessLogQuery(c *gin.Context) *gorm.DB {
	...
	if application := strings.TrimSpace(c.Query("applicationName")); application != "" {
		query = query.Where("application_name LIKE ?", "%"+application+"%")
	}
